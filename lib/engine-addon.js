  ...
  extend: function(options) {
    ...
    options.init = function() {
      ...
      this.compileLazyEngineStyles = function compileLazyEngineStyles(vendorTree, externalTree) {
        ...
        let shouldMinifyCSS = 'enabled' in this.options.minifyCSS ? this.options.minifyCSS.enabled : process.env.EMBER_ENV === 'production';
        ...
        if (engineStylesTree) {
          ...
          primaryStyleTree = this.debugTree(primaryStyleTree, 'engine-style:post-concat');

          if (shouldMinifyCSS) {
            primaryStyleTree = preprocessMinifyCss(primaryStyleTree, minificationOptions);
          }
        ...
        if (shouldMinifyCSS) {
          vendorCSSImportTree = preprocessMinifyCss(vendorCSSImportTree, minificationOptions);
        }
    ...
    */
    options.config = options.config || function() {
      return null;
    };
    ...
    */
    options.contentFor = function(type, config) {
      ...
      content = this.addons.reduce((result, addon) => {
        let addonContent = addon.contentFor ? addon.contentFor(type, config, result) : null;
        if (addonContent) {
          return result.concat(addonContent);
        }
